...
LRESULT Application::Handle_WM_SIZE(WPARAM wParam, LPARAM lParam)
{
	...
	m_deviceResources->CreateWindowSizeDependentResources(m_width, m_height);
	m_swapChain = m_deviceResources->GetSwapChain();
	Update();
...
{
	ComplexType delt = 0.1;
	switch (wParam)
	{
	case VK_F1:		m_c.real=0; m_c.image = 0; break;
	case VK_UP:		m_c.real += delt; break;
	case VK_DOWN:	m_c.real -= delt; break;
	case VK_RIGHT:	m_c.image += delt; break;
	case VK_LEFT:	m_c.image -= delt; break;
	default:return 0;
	}
	GenerateJulia();
	Update();
...
HWND Application::m_hDlg, Application::m_hProcess, Application::m_hStaticText;
INT_PTR Application::DialogProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_INITDIALOG:
		m_hDlg = hDlg;
		m_hProcess = GetDlgItem(m_hDlg, IDC_PROGRESS);
		m_hStaticText = GetDlgItem(m_hDlg, IDC_STATIC_TEXT);
		return (INT_PTR)TRUE;
		break;
	case WM_CLOSE:
		EndDialog(hDlg, 0);
